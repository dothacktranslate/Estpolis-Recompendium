 ...
 */
enum {
    ESTPOLIS1_RESET_PC = 0x008000u,
    ESTPOLIS1_NMI_PC   = 0x008201u,
    ESTPOLIS1_IRQ_PC   = 0x008678u,
};

/* NTSC SNES frame duration in master clocks. */
#define ESTPOLIS1_MASTER_CLOCKS_PER_FRAME 357368ull
...
void Estpolis1RuntimeInitialize(void) {
    s_started = false;
    s_resume_pc = ESTPOLIS1_RESET_PC;
...
static bool Estpolis1RunToBoundary(uint32_t entry_pc) {
    const uint64_t deadline =
        g_cpu.master_cycles + ESTPOLIS1_MASTER_CLOCKS_PER_FRAME;
    ...
    if (!ok) {
        fprintf(
            stderr,
            "[estpolis1] LLE boundary failed at/after $%06X "
...
static bool Estpolis1RunInterrupt(uint32_t vector_pc) {
    /*
...
void Estpolis1RunOneFrame(void) {
    if (!s_started) {
        /*
        ...
        fprintf(
            stderr,
            "[estpolis1] starting untouched hybrid LLE/AOT boot "
            "at $%06X\n",
            ESTPOLIS1_RESET_PC);

        (void)Estpolis1RunToBoundary(ESTPOLIS1_RESET_PC);
        return;
    }
    ...
    if (g_snes && g_snes->nmiEnabled) {
        ...
        if (!Estpolis1RunInterrupt(ESTPOLIS1_NMI_PC))
            return;

        s_nmis++;
    }

    (void)Estpolis1RunToBoundary(s_resume_pc);
}

void Estpolis1DrawPpuFrame(void) {
    ...
    for (int line = 0; line <= 224; line++) {
        ...
        if (line == trigger) {
            ...
            if (Estpolis1RunInterrupt(ESTPOLIS1_IRQ_PC))
                s_irqs++;
...
void Estpolis1PrintDiagnostics(void) {
    long tier_hits = interp_tier_hit_count();

    fprintf(
        stderr,
        "[estpolis1] f=%d resume=$%06X %s "
